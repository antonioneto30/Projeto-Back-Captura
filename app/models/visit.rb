class Visit < ApplicationRecord
  ...
  def date_is_valid?
    if date == nil
      errors.add(:date, "Coloque uma data")
  ...
  def checkin_checkout_valid?
    if checkin_at == nil
      errors.add(:checkin_at, "Coloque a data de check-in.")
    elsif checkout_at == nil
      errors.add(:checkout_at, "Coloque a data de check-out.")
  ...
  def user_is_valid?
    if user_id == nil
      errors.add(:user_id, "Coloque um ID")
    elsif !User.exists?(user_id)
      errors.add(:user_id, "Usuário não foi encontrado")
    end
  end

  def status_is_valid?
    if status == nil
      errors.add(:status, "Coloque um status.")
    elsif status != "TRABALHANDO" and status != "TRABALHADO" and status != "PENDENTE"
