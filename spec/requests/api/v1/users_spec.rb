...
RSpec.describe 'Users', type: :request do
    ...
    before(:all) do 
        @user = User.create!(name: 'Taciana', password: 'taci42', password_confirmation: 'taci42', email: 'taci@gmail.com', cpf: '74658395463')
    end
    ...
    describe '#create' do
        it 'when the user is created' do 
            post '/users', params: {:name => 'Teste', :password_digest => 'Teste42', :email => 'teste@gmail.com', :cpf => '89473695732'}
            expect(response).to be_successful
    ...
    describe '#update' do 
        it 'when the user is updated' do
            authentication = AuthenticateUser.call(@user.email, @user.password)
            put "/users/#{@user.id}", params: {:cpf => '89473695732'}, headers: {"Authorization" => "Bearer #{authentication.result}"}
            @user.reload
            expect(response).to have_http_status(200)
        end
        ...
        context 'when the user is not updated' do 
            it 'when the user is not authenticated' do 
                put "/users/#{@user.id}", params: {:cpf => '89473695732'}
                expect(response).to have_http_status(401)
            end
        end
    end
